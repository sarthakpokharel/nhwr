...
export class PivotComponent implements OnInit {
  ...
  constructor(private RS: PivotService, private toastr: ToastrService, private fb: FormBuilder) { 
...
getFilter(row:any){
  if(row=="province"){
    this.filters=this.provinces;
  }
  else if(row=="district"){
    this.filters=this.district;
  }
  else if(row=="palika"){
    this.filters=this.palika;
  }else{
    this.filters=[];
  }
}
  ...
  getReportsView(row:any,column:any,filter:any,type:any){
  //  alert(filter);
    if (this.groupForm.valid) {
      window.open(this.url+"/pivotview?row="+row+"&column="+column+"&filter="+filter+"&type="+type, '_blank');
    } else {
      Object.keys(this.groupForm.controls).forEach(field => {
        // console.log(field);
        const singleFormControl = this.groupForm.get(field);
  ...
  getReports(row:any,column:any,filter:any,type:any){
    if (this.groupForm.valid) {
      window.location.href = this.url+"/pivotexcel?row="+row+"&column="+column+"&filter="+filter+"&type="+type;
    } else {
      Object.keys(this.groupForm.controls).forEach(field => {
        // console.log(field);
        const singleFormControl = this.groupForm.get(field);
